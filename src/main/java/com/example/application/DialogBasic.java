...
public class DialogBasic extends Div {
    ...
    private static VerticalLayout createDialogLayout(Dialog dialog) {
        ...
        headline.getStyle().set("margin", "var(--lumo-space-m) 0 0 0").set("font-size", "2em").set("font-weight", "italic");

        Button teacher = new Button("Teacher", buttonClickEvent -> {
            UI.getCurrent().navigate(TeacherRegistration.class);
            dialog.close();
        });
        Button student = new Button("Student", buttonClickEvent -> {
            UI.getCurrent().navigate(StudentRegistration.class);
            new RouterLink("student", StudentRegistration.class);
            dialog.close();
        });
